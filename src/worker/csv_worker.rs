...
pub fn save_csv_worker(path: &str, filename: &str, records: &Option<Vec<HashMap<String, String>>>) -> csv::Result<()> {
    ...
    if records.is_none() {
        ...
        if let Some(_first) = &records.as_ref().unwrap().first() {
            writer.write_record(headers)?; // ✅ Added & for write_record

            for row in records.as_ref().unwrap() {
